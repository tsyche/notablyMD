...
import com.tsyche.notablymd.utils.NoteRepairUtils.truncateBodyAndFixSpans
import com.tsyche.notablymd.utils.NoteSplitUtils.splitOversizedExistingNoteForMigration
...
suspend fun Application.runMigrations(onProgressTitle: (Int) -> Unit = {}): Boolean {
    ...
    withContext(Dispatchers.IO) {
        if (dataSchemaId < 1) {
            onProgressTitle(com.tsyche.notablymd.R.string.migration_moving_attachments)
            moveAttachments(preferences)
            newDataSchemaId = 1
            didWork = true
        }
        if (newDataSchemaId < 2) {
            onProgressTitle(com.tsyche.notablymd.R.string.migration_splitting_notes)
            splitOversizedNotes()
            newDataSchemaId = 2
            didWork = true
        }
