package com.philkes.notablymd.presentation.activity
...
abstract class LockedActivity<T : ViewBinding> : AppCompatActivity() {
    ...
    private lateinit var notablymdApplication: NotablyMDApplication
    private lateinit var biometricAuthenticationActivityResultLauncher:
        ActivityResultLauncher<Intent>

    protected lateinit var binding: T
    protected lateinit var preferences: NotablyMDPreferences
    val baseModel: BaseNoteModel by viewModels()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        notablymdApplication = (application as NotablyMDApplication)
        preferences = NotablyMDPreferences.getInstance(application)
    ...
    override fun onPause() {
        super.onPause()
        if (preferences.isLockEnabled && notablymdApplication.locked.value) {
            hide()
        }
    }
    ...
    private fun unlock() {
        notablymdApplication.locked.value = false
        show()
    }
    ...
    private fun hasToAuthenticateWithBiometric(): Boolean {
        return ContextCompat.getSystemService(this, KeyguardManager::class.java)?.let {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
                (it.isDeviceLocked || notablymdApplication.locked.value)
            } else {
                false
            }
        } ?: false
