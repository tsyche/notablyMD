...
import com.philkes.notablymd.utils.NoteRepairUtils.truncateBodyAndFixSpans
import com.philkes.notablymd.utils.NoteSplitUtils.splitOversizedExistingNoteForMigration
...
suspend fun Application.runMigrations(onProgressTitle: (Int) -> Unit = {}): Boolean {
    ...
    withContext(Dispatchers.IO) {
        if (dataSchemaId < 1) {
            onProgressTitle(com.philkes.notablymd.R.string.migration_moving_attachments)
            moveAttachments(preferences)
            newDataSchemaId = 1
            didWork = true
        }
        if (newDataSchemaId < 2) {
            onProgressTitle(com.philkes.notablymd.R.string.migration_splitting_notes)
            splitOversizedNotes()
            newDataSchemaId = 2
            didWork = true
        }
...
private fun Application.moveAttachments(preferences: NotablyMDPreferences) {
